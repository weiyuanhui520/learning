...
def getsize(driver):
    ...
    # x=driver.manage().window().getSize().height
    # y=driver.manage().window().getSize().width
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # fristfriend = driver.find_elements_by_id('com.tencent.mm:id/ft6')
        for ff in range(len(fristfriend)):
            print(len(fristfriend))
            fristfriend[ff].click()
            time.sleep(0.5)
            id1 = 'com.tencent.mm:id/bd_'
            nm = driver.find_element_by_id(id1).get_attribute('name')
            iem = is_element_exist(driver, id1)
            if iem:
                if nm not in remark:
                    remark.append(nm)
            wait.until(EC.element_to_be_clickable((By.ID, "com.tencent.mm:id/ei"))).click()
            time.sleep(1)
            print(remark)
